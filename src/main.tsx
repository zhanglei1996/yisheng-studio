...
const styleNonce = document.querySelector<HTMLStyleElement>("head style[nonce]")?.nonce.trim();
const csp = styleNonce ? { nonce: styleNonce } : undefined;

const appTheme = {
  algorithm: theme.darkAlgorithm,
  token: {
    colorPrimary: "#2f7dff", colorBgBase: "#0b0d10", colorBgContainer: "#12161b",
    colorBgElevated: "#171c22", colorBorder: "#2a3038", colorText: "#f3f5f7",
    colorTextSecondary: "#9ca3af", borderRadius: 6, controlHeight: 34,
...
ConfigProvider.config({ holderRender: (children) => <ConfigProvider csp={csp} locale={zhCN} theme={appTheme}>{children}</ConfigProvider> });
message.config({ duration: 4, maxCount: 3, top: 18 });
