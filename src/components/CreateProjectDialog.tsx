...
import { Alert, Button, Checkbox, Input, Radio, Select, Steps, Tooltip, message } from "antd";
...
export function CreateProjectDialog({ open, onClose, onComplete }: { open: boolean; onClose: () => void; onComplete: (options: ProjectCreationOptions) => void | Promise<void> }) {
  ...
  useEffect(() => {
    if (!open) return;
    setStep(0); setMode("quick"); setAudio("duck"); setRights(false); setProbe(null); setTtsProviderId(null); setTtsVoiceId(null); setProjectName("");
  }, [open]);
  useEffect(() => {
