...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  history: [],
  future: [],
  loadSegments: (segments) => set({ segments, selectedId: segments[0]?.id ?? "", playheadMs: segments[0]?.startMs ?? 0, history: [], future: [] }),
  ...
  updateSegment: (id, patch) => set((state) => withSnapshot(state, state.segments.map((segment) => segment.id === id ? { ...segment, ...patch, status: patch.status ?? "stale" } : segment))),
